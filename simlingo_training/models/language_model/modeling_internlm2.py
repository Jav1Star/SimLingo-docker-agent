...
""" 参考 modeling_ada_llama.py 中的逻辑进行修改。
在推理阶段（Inference）根据调度器（Scheduler）的指令，选择需要的神经网络权重 """
...
""" # Modified InternLM2Model to support L-Mode skipping
修改InternLM2Model以实现L-Mode跳过功能 """
@add_start_docstrings(
    "The bare InternLM2 Model outputting raw hidden-states without any specific head on top.",
    InternLM2_START_DOCSTRING,
)
class InternLM2Model(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        scheduler: Optional[object] = None, # 接收调度器 Callable
        ...
        for idx, decoder_layer in enumerate(self.layers):
            # === [AdaLLaVA Skipping 逻辑] ===
            # === [关键修改 2] 计划生成 (Plan Generation) ===
            # 只有到了锚点层 (Anchor Layer)，且有 Scheduler 和 Latency 时才生成
            if idx == num_prefix_layers:
                if latency is not None and scheduler is not None:
                    # 1. 找到 Latency Token 的特征
                    if latency_token_position is not None:
                        # latency_token_position: [Batch]
                        batch_indices = torch.arange(hidden_states.size(0), device=hidden_states.device)
                        # 提取特征: [Batch, Hidden_Size]
                        latency_token_feat = hidden_states[batch_indices, latency_token_position]
                        
                        # 2. 调用 Scheduler 生成计划
                        # 假设 scheduler 返回 [Batch, Layers, 1]
            ...
            # 只有在过了前缀层之后，且有计划时，才考虑跳过
            if execution_plan is not None and idx >= num_prefix_layers:
                # 获取当前层的指令
                # 注意：Scheduler 输出的 plan 长度通常是从 prefix layer 开始算的，还是全长的？
                # AdaLLaVA 的 SimpleScheduler 通常输出的是全长 Mask (Total Layers)
                # 所以直接用 idx 索引即可
                if idx < len(execution_plan):
                    drop_states = execution_plan[idx]

            # 核心跳过逻辑：如果不是训练模式，且 drop_states 全为 0 (丢弃)
...
# Modified InternLM2ForCausalLM to accept AdaLLaVA args
""" 修改InternLM2ForCausalLM以接受AdaLLaVA参数 """
class InternLM2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # 占位符，实际使用时 VLM wrapper 会注入或在 forward 中传入
        self.scheduler = None 
    ...
    def forward(
        ...
        latency: Optional[torch.FloatTensor] = None,
        latency_token_position: Optional[torch.LongTensor] = None,
        scheduler: Optional[object] = None, # 如果外部不传，尝试使用 self.scheduler
    ) -> Union[Tuple, CausalLMOutputWithPast]:
        """
        此处是为了修复 transformers 旧版本 docstring split None 的 bug 而手动添加的注释。
        Returns:
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
