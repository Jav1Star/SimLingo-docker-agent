...
class LingoAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        state_dict = torch.load(self.config_path)
        if hasattr(self.model, "remap_legacy_state_dict_keys"):
            state_dict = self.model.remap_legacy_state_dict_keys(state_dict)
        self.model.load_state_dict(state_dict, strict=True) ## TODO: adallava训练完成后，推理时应该严格加载
        self._apply_eval_budget_config_to_model()
    ...
    def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        tick_data = self.tick(input_data)
        if self.eval_budget_mode == "rule_based":
    ...
    def _apply_eval_budget_config_to_model(self):
        if not hasattr(self, "model"):
            return
        assigner = getattr(self.model, "budget_assigner", None)
        if assigner is None:
            return

        assigner.set_policy(
            mode=self.eval_budget_mode,
            fixed_budget=float(np.clip(self.fixed_eval_budget, 0.0, 1.0)),
            rule_based_cfg=self.rule_based_cfg if self.eval_budget_mode == "rule_based" else None,
            reset_state=True,
        )

        if hasattr(self.model, "eval_budget"):
            self.model.eval_budget = assigner.get_eval_budget()

    def _load_rule_based_budget_cfg(self):
        cfg_raw = os.getenv("SIMLINGO_EVAL_RULE_BASED_CFG_JSON", "").strip()
        if not cfg_raw:
            raise ValueError("SIMLINGO_EVAL_RULE_BASED_CFG_JSON is required when budget mode is rule_based")
        try:
            user_cfg = json.loads(cfg_raw)
        except Exception:
            raise ValueError("SIMLINGO_EVAL_RULE_BASED_CFG_JSON must be valid JSON dict")
        if not isinstance(user_cfg, dict) or not user_cfg:
            raise ValueError("SIMLINGO_EVAL_RULE_BASED_CFG_JSON must be a non-empty JSON dict")
        return user_cfg

    def _load_fixed_eval_budget(self):
        value = os.getenv("SIMLINGO_EVAL_FIXED_BUDGET", "1.0")
        try:
            value = float(value)
        except Exception:
            value = 1.0
